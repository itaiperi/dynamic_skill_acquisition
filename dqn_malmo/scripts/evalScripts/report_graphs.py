...
def read_agent_instances_file(file_path, file_name, success_rate=False):
...
# Bar chart of success rate of all tasks, averaged over 3 trained agents, of each type of distilled agent
scores = []
errs = []
for cooker_path in cookers_paths[:-1]:
    # We only look at distilled, we don't look at regular
    mine_score = np.mean(read_agent_instances_file(cooker_path, 'scores_mine.t7', True), axis=1)
    hunt_score = np.mean(read_agent_instances_file(cooker_path, 'scores_hunt.t7', True), axis=1)
    cook_score = np.mean(read_agent_instances_file(cooker_path, 'scores_cook.t7', True), axis=1)
    scores_3_agents = np.stack([mine_score, hunt_score, cook_score], axis=0)
    scores.append(np.mean(scores_3_agents))
    errs.append(np.std(np.mean(scores_3_agents, axis=0)))
mine_score = np.mean(read_agent_instances_file(miner_path, 'scores_mine.t7', True), axis=1)
hunt_score = np.mean(read_agent_instances_file(hunter_path, 'scores_hunt.t7', True), axis=1)
cook_score = np.mean(read_agent_instances_file(cookers_paths[-1], 'scores_cook.t7', True), axis=1)
scores_3_agents = np.stack([mine_score, hunt_score, cook_score], axis=0)
scores.append(np.mean(scores_3_agents))
errs.append(np.std(np.mean(scores_3_agents, axis=0)))
...
for i, v in zip(np.arange(5), scores):
    plt.gca().text(i-0.12, v + 7, str(int(v)))
plt.xticks(np.arange(6), cookers_legend[:-1] + ('baselines',), rotation=18)
plt.title('Success rate of all tasks, for all agents')
plt.ylabel('Success Rate')
plt.xlabel('Agent')
plt.gcf().subplots_adjust(bottom=0.23)
plt.savefig('success_rate_all_tasks.png')
...
# Bar chart of success rate per task, averaged over 3 trained agents, of each type of distilled agent. Also show scores of
# regular agents
scores = []
errs = []
plt.figure()
...
plt.xlabel('Agent')
plt.gcf().subplots_adjust(bottom=0.23)
plt.savefig('success_rate_per_task.png')
